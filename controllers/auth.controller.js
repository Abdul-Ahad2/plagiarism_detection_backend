...
import { sendOtpToEmail, verifyOtp } from "../service/otp.service.js";
  ...
  try {
    ...
    if (action === "request") {
      ...
      if (!user)
        return res
          .status(404)
          .json({ error: stringConstants.PASSWORD_RESET.USER_NOT_FOUND });
      await sendOtpToEmail(email);
      return res
        .status(200)
        .json({ message: stringConstants.PASSWORD_RESET.REQUEST_SUCCESS });
    }
    ...
    if (action === "verify") {
      ...
      return res
        .status(200)
        .json({ message: stringConstants.PASSWORD_RESET.VERIFY_SUCCESS });
    }
    ...
    if (action === "reset") {
      ...
      return res
        .status(200)
        .json({ message: stringConstants.PASSWORD_RESET.RESET_SUCCESS });
    }

    return res
      .status(400)
      .json({ error: stringConstants.PASSWORD_RESET.UNKNOWN_ACTION });
  } catch (error) {
...
export const login = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: stringConstants.LOGIN.SUCCESS, token, user });
  } catch (error) {
